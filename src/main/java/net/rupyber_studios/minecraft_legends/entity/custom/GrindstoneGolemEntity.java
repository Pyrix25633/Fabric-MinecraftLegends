...
public class GrindstoneGolemEntity extends ModAbstractGolemEntity implements IAnimatable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if(event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.grindstone_golem.walk", true));
            return PlayState.CONTINUE;
        }
        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.grindstone_golem.idle", true));
        return PlayState.CONTINUE;
    }
